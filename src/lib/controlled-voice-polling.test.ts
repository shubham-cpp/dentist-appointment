...
test("retries only a temporary controlled voice gateway failure", () => {
  ...
  assert.equal(nextControlledVoicePollDelay("gateway_unavailable", 4), 8_000);
  assert.equal(
    nextControlledVoicePollDelay("gateway_unavailable", MAX_CONTROLLED_VOICE_POLL_RETRIES),
    undefined,
  );
